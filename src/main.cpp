...
#if defined(NDEBUG)
# error "Vectorcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa47cdc284a6bc8af644d00711c40ccc9dda855217179134f6fd16dbd49916742");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vectorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vectorcoin Signed Message:\n";
...
{
    // Vectorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 100000 blocks, which will occur approximately every 5 weeks
    nSubsidy >>= (nHeight / 100000); // Vectorcoin: 100k blocks in 5 weeks 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Vectorcoin: 1 day
static const int64 nTargetSpacing = 30; // Vectorcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vectorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vectorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Vectorcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6de1f3dd289e6dec81e625660268c57194aa71bd074bd1a541ac5df3095c2aa4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 10/16/2014 Steep Sell-Off Spreads Fear to Wall Street";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1413517921;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5833225;

        if (fTestNet)
        {
            block.nTime    = 1407913114;
            block.nNonce   = 2687501;
        }
    ...
    // If genesis block hash does not match, then generate new genesis hash.
        ...
        assert(block.hashMerkleRoot == uint256("0x74995319137f860fd9b187d0ca5bf8bec31523836957b5913a9aee14b328ea80"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Vectorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VectorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VectorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VectorcoinMiner(CWallet *pwallet)
{
    printf("VectorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vectorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VectorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VectorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VectorcoinMiner, pwallet));
}
